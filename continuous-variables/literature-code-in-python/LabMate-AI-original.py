...
array = train.values
# array
X = array[:,1:-1] #从第2列到倒数第二列，注意up to but not including
# X
Y = array[:,-1]
# Y #Y是最后一列数据
...
unseen = pd.concat([df_all_combos, df_train_corrected]).drop_duplicates(keep=False) #把all和train合并，去重
array2 = unseen.values #去重后的数据传入array2
X2 = array2[:,1:] #X2的数据为： 合并后的数据，从第二列到最后一列，都是已知参数
df_all_combos2 = df_all_combos.iloc[:,1:] #不要第一列pyridine 不知道为什么？？？
